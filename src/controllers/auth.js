...
exports.signin = async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      if (isPasswordMatch) {
        const { _id, name, email, role, from, profilePicture, dateOfBirth } =
          existingUser;
        const token = await generateJwtToken(_id, email, role);
        // response token and user info
        res.status(200).json({
...
exports.isUserLoggedIn = async (req, res) => {
  try {
    ...
    const { _id, name, email, role, profilePicture, dateOfBirth, from } =
      userObj;
    const user = { _id, name, email, role, profilePicture, dateOfBirth, from };
