...
exports.addProduct = (req, res) => {
  const { name, price, description, category, brand, discountPercent } =
    req.body;
...
exports.updateDiscountPercent = (req, res) => {
  ...
  if (type === "category") {
    ...
    Product.updateMany({ brand: _id }, { $set: { discountPercent } }).exec(
      (error, result) => {
        if (error) return res.status(400).json({ error });
        res.status(202).json({ result });
      }
    ...
    Product.updateOne({ _id }, { $set: { discountPercent } }).exec(
      (error, result) => {
        if (error) return res.status(400).json({ error });
        res.status(202).json({ result });
      }
